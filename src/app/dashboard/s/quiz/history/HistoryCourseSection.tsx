  ...
  return (
    <div className='flex flex-col gap-3 p-2' id={course?.name!}>
        ...
        <section className="flex flex-wrap gap-4 max-sm:items-center">
            {
                quizzes.map(quiz => (
                    <Card key={quiz.id + Math.random()} className='w-72 min-h-44 flex flex-col gap-3 p-4 max-sm:w-full hover:opacity-60 hover:transition-all hover:animate-out' as={Link} href={`/dashboard/s/quiz/history/${quiz.id}`}>

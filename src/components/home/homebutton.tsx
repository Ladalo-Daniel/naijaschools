        ...
        <ul className='flex flex-row gap-2 items-center justify-start md:flex-row flex-wrap md:flex-nowrap'>
          {stats.map((item, idx) => (
            <VisibilitySensor key={idx} partialVisibility onChange={(isVisible: boolean ) => setIsVisible(isVisible)}>
              <li className="w-[45%] md:w-1/2 text-center bg-orange-50 px-7 md:px-12 py-1  md:py-2 rounded-lg sm:w-auto">
                <h4 className="text-lg md:text-2xl text-red-400">{item.data}k+</h4>
                <p className="mt-1 md:mt-2 text-gray-900 font-medium">{item.title}</p>
              </li>
            </VisibilitySensor>
          ))}
        </ul>
